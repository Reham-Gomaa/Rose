...
import { Address, AddressRes } from "@rose/core_interfaces/user-address.interface";
...
export class AddressEffect{
    ...
    readonly getAllAddresses$ = createEffect((): Observable<Action> =>
    this._actions$.pipe(
      ofType(showAddresses),
      switchMap(() => {
        return this._userAddressService.getAllUserAddress<AddressRes>().pipe(
