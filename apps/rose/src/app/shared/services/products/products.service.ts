...
import {
  reviewData,
  reviewsResponse,
} from "../../../features/pages/details/interfaces/review.interface";
...
export class ProductsService {
  ...
  getAllProducts(categoryId?: string): Observable<ProductRes> {
    ...
    return this.httpClient.get<ProductRes>(`${EndPoint.PRODUCTS}`, { params }).pipe(shareReplay(1));
  }
  ...
  getSpecificProduct(id: string): Observable<ProductDetailsRes> {
    ...
    return this.httpClient
      .get<ProductDetailsRes>(`${EndPoint.PRODUCTS}/${id}`)
      .pipe(shareReplay(1));
  }
}
